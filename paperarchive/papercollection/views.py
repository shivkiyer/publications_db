...
class EditPaper(BaseView):
    template_name = "edit_paper.html"

    def extract_paper_data(self, *args, **kwargs):
        paper_srno = int(kwargs['paper_srno'])
        # New paper
        paper_item = Paper.objects.get(id = paper_srno)
        ...
        return [paper_item, author_list, ]

    def copy_paper(self, request, *args, **kwargs):
        paper_srno = int(kwargs["paper_srno"])
        ...
        for author_in_paper in author_list:
            author_position = author_in_paper.contributor_set.\
                                    get(paper = original_paper).position
            paper_contributor = Contributor(paper = edit_paper,
                                            author = author_in_paper,
                                            position = author_position)
            paper_contributor.save()
            edit_paper.save()
        return [original_paper, edit_paper, author_list, ]
    ...
    def get_context_data(self, request, *args, **kwargs):
        ...
        if not request.method == "POST":
            ...
            if "edit_paper" in request.POST:
                original_paper, edit_paper, author_list = \
                                self.copy_paper(request, \
                                                paper_srno=paper_srno)
                original_paper_srno = original_paper.id
            else:
                ...
                edit_paper, author_list = self.extract_paper_data(paper_srno=paper_srno)
                ...
                if journal_submitted.is_valid():
                    ...
                    edit_paper.paper_journal.name = journal_received["name"]
                    if journal_received["organization"]:
                        edit_paper.paper_journal.organization = journal_received["organization"]
                    edit_paper.paper_journal.save()
